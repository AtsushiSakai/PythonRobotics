...
def main():
    ...
    fences = [
        {'name': 'Fence 1', 'lat': 51.71106, 'lon': 0.47100, 'radius': 0.0041477},  # Approx 530 meters
        {'name': 'Fence 2', 'lat': 51.719071, 'lon': 0.4772688, 'radius': 0.001471},  # Approx 525 meters
        {'name': 'Fence 3', 'lat': 51.725, 'lon': 0.485, 'radius': 0.00648}  # Approx 500 meters
    ]
    ...
    ax.set_xlim(51.7, 51.74)  # Set x-axis limits
    ax.set_ylim(0.47, 0.49)  # Set y-axis limits
        ...
        while True:
            for smoothedPath in all_smoothed_paths:
                for point in smoothedPath:
                    location = LocationGlobalRelative(point[0], point[1], 37)  # Example altitude
                    print(f"Leader moving to: {location}")
                    vehicle2.simple_goto(location)
                    time.sleep(10)  # Adjust this delay as necessary to maintain distance
