...
from PathPlanning.TimeBasedPathPlanning import PriorityBasedPlanner as m2
...
@pytest.mark.parametrize("single_agent_planner", [SpaceTimeAStar, SafeIntervalPathPlanner])
def test_better_than_priority_based_planner(single_agent_planner: SingleAgentPlanner):
    """
    Test that CBS returns a better solution than the priority based planner
    """
    grid_side_length = 21

    start_and_goals = [StartAndGoal(i, Position(1, i), Position(19, 19-i)) for i in range(1, 3)]
    obstacle_avoid_points = [pos for item in start_and_goals for pos in (item.start, item.goal)]
    ...
    paths: dict[int, NodePath]
    paths = m2.PriorityBasedPlanner.plan(grid, start_and_goals, single_agent_planner, False)

    # Re-initialize grid to clear reservations that PBP populates
    grid = Grid(
        np.array([grid_side_length, grid_side_length]),
        num_obstacles=250,
        obstacle_avoid_points=obstacle_avoid_points,
    ...
    paths2: dict[int, NodePath]
    paths2 = m.ConflictBasedSearch.plan(grid, start_and_goals, single_agent_planner, False)

    # All paths should start at the specified position and reach the goal
    for start_and_goal in start_and_goals:
        assert paths[start_and_goal.agent_id].path[0].position == start_and_goal.start
        assert paths[start_and_goal.agent_id].path[-1].position == start_and_goal.goal
        assert paths2[start_and_goal.agent_id].path[0].position == start_and_goal.start
        assert paths2[start_and_goal.agent_id].path[-1].position == start_and_goal.goal

    pbp_cost = sum([path.goal_reached_time() for path in paths.values()])
    cbs_cost = sum([path.goal_reached_time() for path in paths2.values()])
    assert pbp_cost > cbs_cost, f"CBS cost {cbs_cost} should be less than PBP cost {pbp_cost}"
