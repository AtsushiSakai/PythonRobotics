...
class Clusters:
    ...
    def __init__(self, x, y, nlabel):
        ...
        self.nlabel = nlabel
        self.labels = [np.random.randint(0, nlabel)
...
def update_clusters(clusters):
    ...
    for ip in range(clusters.ndata):
        ...
        dlist = [np.hypot(idx, idy) for (idx, idy) in zip(dx, dy)]
        mind = min(dlist)
...
def calc_raw_data(cx, cy, npoints, rand_d):
    ...
    for (icx, icy) in zip(cx, cy):
        for _ in range(npoints):
            rx.append(icx + rand_d * (np.random.random() - 0.5))
            ry.append(icy + rand_d * (np.random.random() - 0.5))
...
def calc_association(cx, cy, clusters):
    ...
    for ic in range(len(cx)):
        ...
        dlist = [idx**2 + idy**2 for (idx, idy) in zip(dx, dy)]
